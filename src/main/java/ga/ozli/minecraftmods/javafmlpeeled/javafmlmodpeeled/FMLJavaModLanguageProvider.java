...
public class FMLJavaModLanguageProvider implements IModLanguageProvider
{
    ...
    private static class FMLModTarget implements IModLanguageLoader {
        ...
        public <T> T loadMod(final IModInfo info, final ModFileScanData modFileScanResults, ModuleLayer gameLayer)
        {
            ...
            try
            {
                final Class<?> fmlContainer = Class.forName("ga.ozli.minecraftmods.javafmlpeeled.javafmlmodpeeled.FMLModContainer", true, Thread.currentThread().getContextClassLoader());
            ...
            catch (NoSuchMethodException | ClassNotFoundException | InstantiationException | IllegalAccessException e)
            {
                ...
                throw LamdbaExceptionUtils.uncheck(()->LamdbaExceptionUtils.uncheck(()->mle.getConstructor(IModInfo.class, mls, String.class, Throwable.class)).newInstance(info, Enum.valueOf(mls, "CONSTRUCT"), "net.minecraftforge.fml.modloading.failedtoloadmodclass", e));
            }
        }
    }
